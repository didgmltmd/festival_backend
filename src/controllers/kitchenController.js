...
exports.getZoneOrders = async (req, res) => {
  ...
  const result = await all(
    `
      ...
      WHERE zone = $1 AND ${statusWhere(status)}
      ORDER BY order_timestamp ASC, item_index ASC
    `,
    [zone]
  );
...
exports.completeCooking = async (req, res) => {
  ...
  const cookedAt = now();
  await run(
    "UPDATE order_items SET status = 'ready', cooked_at = $1 WHERE order_timestamp = $2 AND item_index = $3 AND status = 'cooking'",
    [cookedAt, timestamp, index]
  );

  const order = await get("SELECT table_number FROM orders WHERE timestamp = $1", [timestamp]);
  const payload = {
    zone: item.zone,
    timestamp,
    itemIndex: index,
